...
class UserModelTestCase(unittest.TestCase):
	...
	def test_invalid_reset_token(self):
		...
		serlf.assertTrue(u2.verify_password('dog'))
		
	def test_valid_email_change_token(self):
		u = User(email='john@example.com', password='cat')
		db.session.add(u)
		db.session.commit()
		token = u.generate_email_change_token('susan@exmaple.org')
	...
	def test_invalid_email_change_token(self):
		...
		db.session.add(u2)
		db.session.commit()
		token1 = u1.generate_email_change_token('joe@example.com')
	...
	def test_roles_and_permissions(self):
		...
		self.assertFalse(u.can(Permission.WRITE_QUESTIONS))
